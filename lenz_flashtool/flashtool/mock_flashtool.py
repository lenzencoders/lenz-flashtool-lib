...
Mock implementation of FlashTool class for testing purposes.
...
import sys
import numpy as np
from typing import List, Literal, Optional, Any, Dict, Union, Tuple, Callable, Type
import logging
import signal
...
from ..biss.crc import biss_crc6_calc
from ..flashtool.uart import UartBootloaderMemoryStates
...
class MockFlashTool:
    ...
    def _setup_default_mock_data(self):
        ...
        self._mock_responses['fw_ver'] = "00010007"
        self._mock_responses['bl_ver'] = "00010002"

        # Bootloader memory state (default: CRC OK)
        self._mock_responses['memory_state'] = UartBootloaderMemoryStates.UART_MEMORYSTATE_FLASH_FW_CRC_OK

        # Current sensor response (hex string, mA)
        self._mock_responses['enc2_current'] = ("e8030000", 1.0)

        # Instant angle (raw 24-bit value)
        self._mock_responses['instant_angle'] = np.random.randint(0, 2**24)

    def _write_to_port(self, data: bytes) -> None:
        """Mock write to port — no-op."""
        logger.debug(f"Mock: Writing {len(data)} bytes to port")
    ...
    def biss_read_calibration_temp_vcc(self, iterations: int = 1) -> None:
        """Mock calibration data reading. Unlike real hardware, defaults to 1 iteration."""
    ...
    def select_spi_channel(self, channel: Literal["channel1", "channel2"]) -> None:
        ...
        logger.debug(f"Mock: Selected SPI channel: {channel}")

    def select_flashtool_mode(self, mode: Literal["spi_spi", "ab_uart", "spi_uart_irs", "ab_spi", "default_spi"]) -> None:
        """Mock FlashTool mode selection."""
        valid_modes = ["spi_spi", "ab_uart", "spi_uart_irs", "ab_spi", "default_spi"]
        if mode not in valid_modes:
            raise ValueError(f'Invalid mode: "{mode}". Must be one of: {valid_modes}')
        logger.debug(f"Mock: Selected FlashTool mode: {mode}")

    def select_FlashTool_current_sensor_mode(self, mode: Literal["disable", "enable"]) -> None:
        """Mock current sensor mode selection."""
        valid_modes = ["disable", "enable"]
        if mode not in valid_modes:
            raise ValueError(f'Invalid mode: "{mode}". Must be one of: {valid_modes}')
        logger.debug(f"Mock: Selected current sensor mode: {mode}")

    def select_spi_ch1_mode(self, mode: Literal["lenz_biss", "lir_ssi", "lir_biss_21b"]) -> None:
        """Mock SPI channel 1 mode selection."""
        valid_modes = ["lenz_biss", "lir_ssi", "lir_biss_21b"]
        if mode not in valid_modes:
            raise ValueError(f'Invalid mode: "{mode}". Must be one of: {valid_modes}')
        logger.debug(f"Mock: Selected SPI CH1 mode: {mode}")

    # ── Data streaming ───────────────────────────────────────────────────

    def read_data_enc1_AB_enc2_SPI(self, read_time: float, status: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """Mock AB + SPI dual encoder reading."""
        status and logger.debug(f'Mock: Reading AB+SPI dual encoder data for {read_time} seconds')
        enc2 = np.random.randint(0, 2**24, 1000, dtype='int32')
        enc1 = np.random.randint(0, 2**16, 1000, dtype='int32')
        return enc2, enc1

    def read_enc2_current(self) -> Union[Tuple[str, float], bool]:
        """Mock encoder channel 2 current reading."""
        logger.debug("Mock: Reading encoder 2 current")
        return self._mock_responses['enc2_current']

    def read_instant_angle_enc_SPI(self) -> Union[Tuple[int, List[int]], bool]:
        """Mock instant angle reading via SPI."""
        angle_value = self._mock_responses['instant_angle']
        angle_deg = angle_value * 360 / 2**24
        degrees = int(angle_deg)
        remaining = angle_deg - degrees
        minutes = int(remaining * 60)
        seconds = int((remaining * 60 - minutes) * 60)
        logger.debug(f"Mock: Instant angle: {degrees}° {minutes}' {seconds}\"")
        return angle_value, [degrees, minutes, seconds]

    def read_instant_angle_packet_enc_SPI(self) -> Union[Tuple[int, List[int]], bool]:
        """Mock instant angle packet reading with CRC6 validation."""
        angle_value = self._mock_responses['instant_angle']
        nE = 1
        nW = 1
        nE_nW_bits = (nE << 1) | nW
        data_for_crc = np.uint32(angle_value << 2) | np.uint32(nE_nW_bits)
        received_crc = biss_crc6_calc(data_for_crc)
        ...
        logger.debug(f"Mock: Instant angle packet: {degrees}° {minutes}' {seconds}\" nE:{nE} nW:{nW} CRC:{received_crc:02X}")
        return angle_value, [degrees, minutes, seconds, nE, nW, received_crc, "OK"]

    # ── Bootloader operations ────────────────────────────────────────────

    def reboot_to_bl(self) -> None:
        """Mock reboot FlashTool to bootloader."""
        logger.info('Mock: Sending REBOOT to BOOTLOADER command to FlashTool')

    def reboot_to_fw(self) -> None:
        """Mock reboot FlashTool to firmware."""
        logger.info('Mock: Sending REBOOT to FIRMWARE command to FlashTool')

    def read_fw_bl_ver(self) -> Tuple[str, str]:
        """Mock firmware and bootloader version reading."""
        fw_ver = self._mock_responses['fw_ver']
        bl_ver = self._mock_responses['bl_ver']
        logger.info(f"Mock: Firmware version: {fw_ver}, Bootloader version: {bl_ver}")
        return fw_ver, bl_ver

    def read_memory_state_bl(self) -> 'UartBootloaderMemoryStates':
        """Mock bootloader memory state reading."""
        state = self._mock_responses['memory_state']
        logger.debug(f"Mock: Memory state: {state.name}")
        return state

    def check_main_fw_crc32(self) -> None:
        """Mock firmware CRC32 check."""
        logger.info('Mock: Sending check main firmware CRC32 command to FlashTool')
        self.read_memory_state_bl()

    def download_fw_to_ft(self, hex_file_path: str, max_retries: int = 3, pbar: Optional[Any] = None) -> None:
        """Mock firmware download to FlashTool device."""
        logger.info(f"Mock: Downloading firmware from {hex_file_path}")
        self.check_main_fw_crc32()

    def enter_bl_biss_encoder(self, reset_attempts: int = 12, power_cycle_delay: float = 0.01) -> bool:
        """Mock enter bootloader mode for BiSS encoder."""
        logger.info("Mock: Entering bootloader mode for BiSS encoder")
        return True

    def enter_bl_irs(self) -> bool:
        """Mock enter bootloader for IRS encoder."""
        logger.info("Mock: Entering bootloader for IRS encoder")
        return True

    def enter_fw_irs(self) -> bool:
        """Mock exit bootloader for IRS encoder."""
        logger.info("Mock: Exiting bootloader for IRS encoder")
        return True

    def set_pos_irs(self, deg: float, reverse: bool = False) -> Tuple[Optional[int], Optional[int]]:
        """Mock set IRS encoder position."""
        if not (0 <= deg <= 360):
            logger.error(f"Mock: Invalid angle {deg}. Must be between 0 and 360 degrees.")
            return None, None
        ...
        logger.debug(f"Mock: Setting IRS position to {deg}° ({'reverse' if reverse else 'forward'})")
        return b_1, b_2
